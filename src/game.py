        ...
        while True:
            ...
            if len(self.event.joysticks) != 0 and not self.event.is_calibrate:
                ...
                if not self.title_screen.getIsTitle():
                    ...
                    display.blit(hud_sheet.subsurface(0, 0, 154, 30),
                                 (display.get_width() / 6, display.get_height() - 40))
                    display.blit(hud_sheet.subsurface(155, 0, 74, 30),
                                 (display.get_width() / 1.5, display.get_height() - 40))
                    self.font_custom.draw_msg(display, [display.get_width() / 6 + 133, display.get_height() - 33],
                                              f'{self.stage_menu.player.coins}') if self.stage_menu.is_menu_stage else self.font_custom.draw_msg(
                        display, [98, 192], f'{self.maps.player.coins}')
                    self.font_custom.draw_msg(display, [display.get_width() / 6 + 105, display.get_height() - 25],
                                              f'{self.stage_menu.player.score}',
                                              True) if self.stage_menu.is_menu_stage else self.font_custom.draw_msg(
                        display, [98, 192], f'{self.maps.player.score}', True)
                    self.font_custom.draw_msg(display, [display.get_width() / 6 + 28, display.get_height() - 25],
                                              f'{self.stage_menu.player.life}') if self.stage_menu.is_menu_stage else self.font_custom.draw_msg(
                        display, [98, 192], f'{self.maps.player.life}')
                    self.font_custom.draw_msg(display, [display.get_width() / 6 + 35, display.get_height() - 33],
                                              self.stage_menu.stage)

                # UPDATES
                if self.title_screen.getIsTitle():
                    self.title_screen.updates(self.dt, self.event.keys_pressed)
                if all([self.title_screen.getPassStageMenu(), self.stage_menu.load_stage_menu,
                        not self.stage_menu.pass_maps]):
