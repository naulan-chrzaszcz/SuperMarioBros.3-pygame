...
class Scene1(object):
    ...
    def start(self, surface: Surface, dt: float) -> None:
        ...
        match int(self.t):
            case 5: self.step[0] = True
            case 20: self.step[0] = False; self.step[1] = True
            case 25: self.step[2] = True
            case 31: self.step[3] = True
            case 35: self.step = [False, True, False, True]
        self.t += (.1 * dt)

        self.titleX -= (cos(self.t/5)*7)*dt if self.step[2] and not (self.titleX+w) < w/3.5 else 0
        self.curtain_y += (1.25 * dt) if self.step[0] else 0
