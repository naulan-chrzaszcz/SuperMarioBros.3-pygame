    ...
    def events(self):
        if self.where == 'Stage_Menu':
            if abs(self.verify_move) != 16:
                ...
                # Keyboard event
                self.move['up'],self.move['down'],self.move['right'],self.move['left'] = self.keys[f'{K_z}'],self.keys[
            ...
            if abs(self.verify_move) >= 16:
                ...
                if all(
                    [any([self.move['up'] == 0,self.move['down'] == 0,self.move['right'] == 0,self.move['left'] == 0]),
                     any([self.keys[f'{K_z}'],self.keys[f'{K_s}'],self.keys[f'{K_d}'],self.keys[f'{K_q}']])]) is False:
                    self.verify_move = 0
        else:
